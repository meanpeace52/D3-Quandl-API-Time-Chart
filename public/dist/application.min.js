...
"use strict";!function(){var toggleSidebar=function($rootScope,$window,Authentication){return{restrict:"A",link:function(scope,element,attrs){scope.$watch(function(){return $rootScope.isToggleSideBar},function(newValue){newValue?element.addClass("toggled"):element.removeClass("toggled")}),scope.$watch(function(){return $window.innerWidth},function(newValue){newValue<768&&$rootScope.isToggleSideBar&&($rootScope.isToggleSideBar=!1,element.removeClass("toggled"))}),scope.$watch(function(){return Authentication.user},function(newValue){newValue?element.addClass("signed-in"):element.removeClass("signed-in")})}}};angular.module("core").directive("toggleSidebar",["$rootScope","$window","Authentication",toggleSidebar])}();
...
"use strict";angular.module("process").factory("Deployr",["$q","$http",function($q,$http){return{run:function(dataset,task){var dfd=$q.defer();return $http.post("/api/deployr/run",{filename:task.script.filename,rinputs:task.script.rInputsFn(dataset.columns,dataset.rows,task.options),routputs:task.script.routputs}).success(function(data,status,headers,config){dfd.resolve(data)}).error(function(data,status,headers,config){dfd.reject(data)}),dfd.promise},runExternal:function(task){},runCode:function(task){}}}]);
