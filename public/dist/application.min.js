...
"use strict";!function(){var capitalize=function(){return function(input,all){var reg=all?/([^\W_]+[^\s-]*) */g:/([^\W_]+[^\s-]*)/;return input?input.replace(reg,function(txt){return txt.charAt(0).toUpperCase()+txt.substr(1).toLowerCase()}):""}};angular.module("core").filter("capitalize",["$rootScope","$window",capitalize])}();
...
"use strict";angular.module("datasets").run(["Menus",function(Menus){}]);
...
"use strict";angular.module("datasets").controller("DatasetsCreateController",["$scope","$state","Authentication","Datasets",function($scope,$state,Authentication,Datasets){var vm=this;vm.authentification=Authentication,vm.create=function(isValid){return vm.error=null,isValid?void vm.dataset.$save(function(response){$state.go("datasets.detail",{datasetId:response._id})},function(errorResponse){vm.error=errorResponse.data.message}):($scope.$broadcast("show-errors-check-validity","postForm"),!1)}}]);
...
"use strict";angular.module("posts").config(["$stateProvider",function($stateProvider){var MODULE_PATH="modules/posts/client/";$stateProvider.state("posts",{"abstract":!0,url:"/posts",template:"<ui-view/>"}).state("posts.list",{url:"",controller:"postsListController",controllerAs:"postsList",templateUrl:MODULE_PATH+"list/posts.list.html"}).state("posts.search",{url:"/search/:field/:value",controller:"postsListController",controllerAs:"postsList",templateUrl:MODULE_PATH+"list/posts.list.html"}).state("posts.create",{url:"/create",controller:"postsCreateController",controllerAs:"postsCreate",templateUrl:MODULE_PATH+"create/posts.create.html",data:{roles:["user","admin"]}}).state("posts.detail",{url:"/:postId",controller:"postsDetailController",controllerAs:"postsDetail",templateUrl:MODULE_PATH+"detail/posts.detail.html"}).state("posts.edit",{url:"/:postId/edit",controller:"postsEditController",controllerAs:"postsEdit",templateUrl:MODULE_PATH+"edit/posts.edit.html",data:{roles:["user","admin"]}})}]);
"use strict";angular.module("posts").controller("postsCreateController",["$scope","$state","Authentication","posts","postOptions","$uibModal","$window","$timeout","FileUploader","Models","Datasets","$log",function($scope,$state,Authentication,posts,postOptions,$uibModal,$window,$timeout,FileUploader,Models,Datasets,$log){var vm=this;vm.submitted=!1,vm.user=Authentication.user,vm.error=null,vm.postOptions=postOptions,vm.post={},vm.addModel=function(model){vm.post.models||(vm.post.models=[]),vm.post.models.push(model)},vm.addDataset=function(dataset){vm.post.datasets||(vm.post.datasets=[]),vm.post.datasets.push(dataset)},vm.create=function(form){vm.submitted=!0,vm.error=null,form.$valid?posts.create(vm.post).then(function(response){$state.go("posts.detail",{postId:response._id})})["catch"](function(err){$log.error(err)}):$scope.$broadcast("show-errors-check-validity","form")},$scope.uploader=new FileUploader({url:"api/users/files"}),$scope.uploader.onAfterAddingFile=function(fileItem){if($window.FileReader){var fileReader=new FileReader;fileReader.readAsDataURL(fileItem._file),fileReader.onload=function(event){$timeout(function(){var pdfData={name:fileItem.file.name,file:event.target.result};$scope.upload(pdfData)},0)}}},$scope.upload=function(file){vm.success=vm.error=null,$scope.uploader.uploadAll()},$scope.cancelUpload=function(){$scope.uploader.clearQueue()},$scope.uploader.onSuccessItem=function(fileItem,response,status,headers){vm.success=!0,vm.post.files||(vm.post.files=[]),vm.post.files.push({name:fileItem.file.name}),$scope.cancelUpload()},$scope.uploader.onErrorItem=function(fileItem,response,status,headers){$scope.cancelUpload(),vm.error=response.message},Models.filter("user",Authentication.user._id).then(function(res){vm.models=res.data},function(error){}),Datasets.user(Authentication.user.username).then(function(res){vm.datasets=res.data},function(error){})}]);
"use strict";angular.module("posts").controller("postsDetailController",["$stateParams","Authentication","posts","$state","$log",function($stateParams,Authentication,posts,$state,$log){var vm=this;vm.notfound=!1,vm.authentication=Authentication,posts.crud.get({postId:$stateParams.postId}).$promise.then(function(response){vm.post=response},function(err){$log.error(err),vm.notfound=!0}),vm.remove=function(){posts.crud.remove({postId:$stateParams.postId}).$promise.then(function(response){$state.go("users.profilepage.posts",{username:vm.authentication.user.username})},function(err){vm.error=err.message})}}]);
"use strict";angular.module("posts").controller("postsEditController",["$scope","$state","$stateParams","Authentication","posts","$uibModal","$window","$timeout","FileUploader","postOptions","Models","Datasets","$log",function($scope,$state,$stateParams,Authentication,posts,$uibModal,$window,$timeout,FileUploader,postOptions,Models,Datasets,$log){var vm=this;vm.user=Authentication.user,vm.submitted=!1,vm.error=null,vm.postOptions=postOptions,vm.post={},vm.get=function(){posts.crud.get({postId:$stateParams.postId}).$promise.then(function(res){vm.post=res},function(err){vm.error=err})},vm.get(),vm.update=function(form){vm.error=null,vm.submitted=!0,form.$valid?posts.crud.update({post:vm.post,postId:$stateParams.postId}).$promise.then(function(response){$state.go("posts.detail",{postId:response._id})},function(err){vm.error=err.message}):$scope.$broadcast("show-errors-check-validity","form")},vm.addModel=function(model){vm.post.models||(vm.post.models=[]),vm.post.models.push(model)},vm.addDataset=function(dataset){vm.post.datasets||(vm.post.datasets=[]),vm.post.datasets.push(dataset)},$scope.uploader=new FileUploader({url:"api/users/files"}),$scope.uploader.onAfterAddingFile=function(fileItem){if($window.FileReader){var fileReader=new FileReader;fileReader.readAsDataURL(fileItem._file),fileReader.onload=function(event){$timeout(function(){var pdfData={name:fileItem.file.name,file:event.target.result};$scope.upload(pdfData)},0)}}},$scope.upload=function(file){vm.success=vm.error=null,$scope.uploader.uploadAll()},$scope.cancelUpload=function(){$scope.uploader.clearQueue()},$scope.uploader.onSuccessItem=function(fileItem,response,status,headers){vm.success=!0,vm.post.files||(vm.post.files=[]),vm.post.files.push({name:fileItem.file.name}),$scope.cancelUpload()},$scope.uploader.onErrorItem=function(fileItem,response,status,headers){$scope.cancelUpload(),vm.error=response.message},Models.filter("user",Authentication.user._id).then(function(res){vm.models=res.data},function(error){}),Datasets.user(Authentication.user.username).then(function(res){vm.datasets=res.data},function(error){})}]);
"use strict";angular.module("posts").controller("postsListController",["$scope","$stateParams","$state","Authentication","posts","UsersFactory",function($scope,$stateParams,$state,Authentication,posts,UsersFactory){var vm=this;vm.authentication=Authentication,vm.state=$state.current.name,vm.ownership=UsersFactory.ownership(),vm.showCreate="posts.list"==$state.current.name,vm.postLimit=10,vm.resolved=!1,vm.loading=!1,vm.load=function(){vm.resolved=!1,vm.loading=!0},vm.loaded=function(){vm.resolved=!0,vm.loading=!1},vm.getPosts=function(field,value){vm.load(),vm.list=[];var getPosts;field?(vm.field=field,vm.value=value,"title"!==vm.field&&(vm.query=""),getPosts=posts.search(field,value)):getPosts=posts.list(),getPosts.then(function(posts){vm.list=posts,vm.loaded()},function(err){vm.loaded()})},vm.search=function(){vm.field="title",vm.query?vm.getPosts(vm.field,vm.query):vm.getPosts()},"posts.list"===vm.state&&(vm.menuItems=["finance","sports","soshsci"],vm.field="subject"),"posts.search"===vm.state&&vm.getPosts($stateParams.field,$stateParams.value),"users.profilepage.posts"===vm.state&&(vm.load(),UsersFactory.userData("posts",$stateParams.username).then(function(res){vm.list=res,vm.loaded()})),"home"===vm.state&&(vm.postLimit=3,vm.menuItems=["trending","in the news","rising"],vm.getPosts("trending",vm.menuItems[0]))}]);
