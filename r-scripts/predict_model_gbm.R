...
deployrInput('{"name":"model", "default":"alex/models/968f38a45fc238b38a42ebc12fedac88dc64d5e5", "render":"character", "label":"Specify link to the model"}')
...
model_yvar <- result$model$response.name
model_xvars <- result$model$var.names
...
if (all(model_xvars %in% xvars) & all(xvars %in% model_xvars)) {
  ...
  .family <- ifelse(is.numeric(newdata[, yvar]), "gaussian", "bernoulli")

  # Reconstructs data
  model_data <- matrix(result$model$data$x, ncol = length(model_xvars), byrow = FALSE)
  colnames(model_data) <- model_xvars
  model_data <- cbind(as.numeric(result$model$data$y), model_data)
  colnames(model_data)[1] <- model_yvar
  model_data <- as.data.frame(model_data)

  # Re-fit model
  model <- gbm(.formula, data = model_data, distribution = .family, n.trees = result$model$n.trees,
               keep.data = TRUE,
               bag.fraction = 0.8,
               interaction.depth = 5,
               shrinkage = 0.1)
...
pred <- predict(result$model, newdata = newdata, n.trees = result$model$n.trees)
